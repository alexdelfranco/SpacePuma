...
class integrate(widget_base):
    ...
    def setup_buttons(self):
        ...
        # Add a button widget to add new points
        self.fit_type_select = widgets.Select(description='Fit Type:',options=['Gaussian','Skew Gaussian'],rows=0,style={'description_width':'initial'},layout = widgets.Layout(width='200px'))
        ...
        return [self.add_range_button,self.adjust_range_button,self.integrate_button,self.fit_type_select,self.fit_order_inttext,self.fit_button,self.clear_axis_button],[self.add_range_button,self.adjust_range_button,self.integrate_button,self.fit_button,self.clear_axis_button]
    ...
    def __call__(self,event):
        ...
        for ax in self.fig.axes:
            ...
            self.update_dictionaries(ax,artist_key='Bounds',data_key='Integration')
        ...
        # If in fit mode
            ...
            if self.info['fit_type'] == 'Gaussian':
                ...
                self.data[fit_ax]['Stats'] = self.gaussian_fit(ax,self.ngaussian_model)
                ...
                self.data[ax]['Fit'][f'Line {fnum}']['ydata'] = self.data[fit_ax]['Stats']['sim_corr']
                # Save the Fit Line data to the original axis
                self.data[fit_ax]['Fit Line']['xdata'] = xdata
                self.data[fit_ax]['Fit Line']['ydata'] = self.data[fit_ax]['Stats']['sim_corr']

                # Plot the fit line
                self.artists[ax][f'Fit Line {fnum}'] = ax.plot(self.data[ax]['Fit'][f'Line {fnum}']['xdata'],self.data[ax]['Fit'][f'Line {fnum}']['ydata'],lw=2,alpha=0.6,color='red')[0]
                ...
                for ii in np.arange(self.info['fit_order']):
                    ...
                    self.data[fit_ax][f'Fit {ii+1}']['ydata'] = self.gaussian(xdata, popt[3*ii + 3], popt[3*ii + 3 + 1], popt[3*ii + 3 + 2])
                    self.artists[fit_ax][f'Fit {ii+1}'] = fit_ax.fill_between(self.data[fit_ax][f'Fit {ii+1}']['xdata'], self.data[fit_ax][f'Fit {ii+1}']['ydata'], color = colors[ii%6], lw = 1, label = f'Model {ii+1}',alpha=0.6)
                    ...
                    if artist == f"Fit Line {self.data[ax]['Fit Number']}":
                        ...
                        if self.data[ax]['Data Axis'] in self.artists_global.keys():
                            ...
                            self.artists_global[self.data[ax]['Data Axis']].pop(artist,None)
                                # del self.artists_global[self.data[ax]['Data Axis']][artist]
    ...
    def integrate(self,ax):
        ...
        # area = scpyintegrate.simps(yspec[ii],xspec[ii])

        # Plot the integration
        self.artists[ax]['Integration'] = ax.fill_between(xdata,ydata,color='dodgerblue',alpha=0.6)
    ...
    def gaussian_fit(self,ax,model):
        ...
        ## Fit products
        # The model only fits the baseline in the selected region,
        # so the corrected data could look weird outside that region
        # because we're using a local baseline for the whole plot

        # Save the baseline of best fit
        baseline = popt[1] + popt[0] * (xdata - popt[2])
        # Save a baseline subtracted data array
        y_corr  = ydata - baseline

        # Create a fit from the xdata and fitted parameters
        simdat = model(xdata,*popt)
        # Create a baseline-subtracted fit
        sim_corr = simdat - baseline
        ...
        stats = {'popt':popt,'perr':perr,'xsub':xsub,'ysub':ysub,'y_corr':y_corr,'sim_corr':sim_corr,'simdat':simdat,'baseline':baseline}
    ...
    def add_fit_stats(self,stats,gnum):
        ...
        for i in range(0,gnum):
            ...
            area[f'fit{i+1}'] = np.sqrt(2*np.pi)*popt[i*3 + 3]*popt[i*3 + 3 + 1]
            # error of the area of the first Gaussian calculated from the errors in the Gaussian parameters
            area_err[f'fit{i+1}_err'] = area[f'fit{i+1}'] * np.sqrt((perr[i*3 + 3]/popt[i*3 + 3])**2 + (perr[i*3 + 3 + 1]/popt[i*3 + 3 + 1])**2)

            # The FWHM (full width half maximum) of the Gaussian can be calculated from the width parameter of the Gaussian
            fwhm[f'fit{i+1}'] = 2.35482 * popt[3*i + 3]
            fwhm_err[f'fit{i+1}_err'] = 2.35482 * perr[3*i + 3]
